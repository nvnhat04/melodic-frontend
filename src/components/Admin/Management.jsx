  ...
  const getNestedValue = (obj, path) => {
    try {
      return path.split(".").reduce((acc, key) => {
        // Handle array indexing, e.g., "artists[0]"
        if (key.includes("[")) {
          const [baseKey, index] = key.split(/[\[\]]/).filter(Boolean);
          return acc[baseKey][parseInt(index, 10)];
        }
        return acc[key];
      }, obj);
    } catch (e) {
  ...
  const scrollToCharacter = (character) => {
    if (character != "") {
      let index = data.findIndex((item) =>
        String(item.id).toLowerCase().startsWith(character.toLowerCase())
      );
      if (index === -1) {
        ...
        <TableBody>
          {paginatedData.map((item) => (
            <TableRow
              ...
              style={{
                backgroundColor:
                  item.id === highlightedItemId ? "yellow" : "transparent",
              }}
            >
              <TableCell padding="checkbox">
                <Checkbox
